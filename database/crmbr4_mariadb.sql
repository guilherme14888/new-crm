-- ============================================================
-- CRM BR4 - Setup completo para MariaDB
-- Banco: crmbr4
-- Compativel com MariaDB 10.2+
-- ============================================================
-- Instrucoes:
--   mariadb -u root -p < database/crmbr4_mariadb.sql
-- ============================================================
...
-- 5. FUNNEL_STAGES
-- ============================================================
CREATE OR REPLACE TABLE funnel_stages (
  id          CHAR(36)     NOT NULL,
  company_id  VARCHAR(36)  NOT NULL DEFAULT '00000000-0000-0000-0000-000000000001',
  ...
  updated_at  DATETIME(3)  NOT NULL DEFAULT CURRENT_TIMESTAMP(3) ON UPDATE CURRENT_TIMESTAMP(3),
  PRIMARY KEY (id),
  CONSTRAINT fk_fs_funnel FOREIGN KEY (funnel_id) REFERENCES funnels(id) ON DELETE CASCADE,
...
CREATE OR REPLACE TABLE deals (
  ...
  closing_reason      TEXT         DEFAULT NULL,
  notes               TEXT         DEFAULT NULL,
...
CREATE OR REPLACE TABLE activities (
  ...
  CONSTRAINT fk_act_contact FOREIGN KEY (contact_id) REFERENCES contacts(id),
...
CREATE OR REPLACE TABLE custom_fields (
  ...
  options         LONGTEXT     DEFAULT NULL CHECK (options IS NULL OR JSON_VALID(options)),
...
-- ============================================================
CREATE OR REPLACE TABLE deal_custom_values (
  id              VARCHAR(36) NOT NULL,
  deal_id         VARCHAR(36) NOT NULL,
...
-- 12. DEAL_PRODUCTS
-- ============================================================
CREATE OR REPLACE TABLE deal_products (
  id              VARCHAR(36)    NOT NULL,
  deal_id         VARCHAR(36)    NOT NULL,
...
-- 13. DEAL_FILES
-- ============================================================
CREATE OR REPLACE TABLE deal_files (
  id              VARCHAR(36)  NOT NULL,
  deal_id         VARCHAR(36)  NOT NULL,
...
-- ============================================================
CREATE OR REPLACE TABLE opportunity_rules (
  id             CHAR(36)     NOT NULL,
  funnel_id      CHAR(36)     NOT NULL,
  name           VARCHAR(255) NOT NULL,
  trigger_type   VARCHAR(50)  NOT NULL,
  trigger_config LONGTEXT     NOT NULL CHECK (JSON_VALID(trigger_config)),
  action_type    VARCHAR(50)  NOT NULL,
  action_config  LONGTEXT     NOT NULL CHECK (JSON_VALID(action_config)),
  is_active      TINYINT(1)   NOT NULL DEFAULT 1,
  created_at     DATETIME(3)  NOT NULL DEFAULT CURRENT_TIMESTAMP(3),
  updated_at     DATETIME(3)  NOT NULL DEFAULT CURRENT_TIMESTAMP(3) ON UPDATE CURRENT_TIMESTAMP(3),
  PRIMARY KEY (id),
  CONSTRAINT fk_or_funnel FOREIGN KEY (funnel_id) REFERENCES funnels(id) ON DELETE CASCADE,
  INDEX idx_funnel (funnel_id)
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci;
...
-- 17. WIN_LOSS_REASONS
-- ============================================================
CREATE OR REPLACE TABLE win_loss_reasons (
  id         CHAR(36)     NOT NULL,
  company_id VARCHAR(36)  NOT NULL DEFAULT '00000000-0000-0000-0000-000000000001',
...
-- 19. USER_COMPANIES
-- ============================================================
CREATE OR REPLACE TABLE user_companies (
  user_id    VARCHAR(36) NOT NULL,
...
-- 20. APP_SETTINGS
-- ============================================================
CREATE OR REPLACE TABLE app_settings (
...
CREATE OR REPLACE TABLE sync_meta (
  ...
  last_pulled_at DATETIME(3) DEFAULT NULL,
...
-- ============================================================
-- FIM - Banco crmbr4 pronto para uso no MariaDB
-- ============================================================
